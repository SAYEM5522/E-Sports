...
const ChatBox = () => {
  ...
  const getMessageList=async()=>{
    const con="63d26e61fb6f620951ca6cd8"
     axios.get(`http://localhost:8081/getMessages/${con}`).then((res)=>{
  ...
  return (
    <div className='h-[90%] w-[340px] bg-[#1C1B22] rounded-md ml-5 relative mt-1'>
      ...
      <div className='h-[83%] flex flex-col   overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden  '>
       ...
       <input type={"text"} onChange={(e:any)=>setMessage(e.target.value)} className="h-10 w-[80%] absolute bottom-1 text-white left-0 right-0 outline-none rounded-md ml-2 mr-2  bg-transparent p-2 border "/>
